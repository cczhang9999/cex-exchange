...
type DepositReq struct {
	...
	Asset  string `json:"asset" v:"required#请输入币种"`
...
func (c *ControllerV1) Deposit(ctx context.Context, req *DepositReq) (res *DepositRes, err error) {
	...
	// TODO: 实现充值逻辑
	g.Log().Infof(ctx, "User %d deposit %s %s", uid, req.Amount, req.Asset)
...
func (c *ControllerV1) ListAccounts(ctx context.Context, req *ListAccountsReq) (res []model.Account, err error) {
	uid := g.RequestFromCtx(ctx).GetCtxVar("uid").Uint64()
